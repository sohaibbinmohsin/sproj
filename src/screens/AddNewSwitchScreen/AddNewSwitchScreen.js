...
const AddNewSwitchScreen = () => {
    ...
    const onAddSwitchPressed = async data => {
        ...
        try{
            ...
            const authCredential = provider.credential(auth().currentUser.email, data.password);
            await firebase.auth().signInWithCredential(authCredential)
            .then(()=>{
                db.collection("switches").where('Id','==', data.switchid).get().then((snapshot)=>{
                    if(snapshot.empty)
                    {
                                ...
                                db.collection("users").where('Id','==', auth().currentUser.email).get().then((snapshot) => {
                                    snapshot.docs.forEach(doc => {
                                        ...
                                        db.collection("users").doc(ID).update({
                                            ...
                                            db.collection("switches").doc(data.switchid).update({
                                                Registered: true
                                            }).then(()=>{
                                                Alert.alert('Success', 'Your switch is configured successfully')
                                                setLoading(false)
                                            }).catch(err => {
    ...
    return(
        ...
        <View style={styles.root}>
            ...
            <CustomButton text={loading ? "Loading... " : "Add Switch"} onPress={handleSubmit(onAddSwitchPressed)} />
